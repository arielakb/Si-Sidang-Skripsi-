...
export default function SeminarHasilHasilPage() {
  ...
  const sidangQuery = useQuery({
    queryKey: ["sidang", "seminar-hasil", "hasil"],
    queryFn: () =>
      getSidangList({
        jenis: "SEMINAR_HASIL",
        limit: 100
  ...
  const hasilMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: ["sidang", "seminar-hasil", "jadwal"]
        }),
        queryClient.invalidateQueries({
          queryKey: ["my-skripsi"]
        }),
      ...
      closeDrawer();
      setPageError("");
      setSuccessMessage("Hasil seminar hasil berhasil disimpan.");
    },
    onError: (error) => {
      setSuccessMessage("");
      setPageError(
        getApiErrorMessage(
          error,
          "Gagal menyimpan hasil seminar hasil. Pastikan Anda penguji aktif pada sidang ini."
        )
      );
    }
  });
    ...
    return (
      <section className="page-stack">
        <div className="alert-error">
          Anda tidak memiliki akses ke halaman hasil seminar hasil.
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="page-stack">
      <PageHeader
        ...
        title="Hasil Seminar Hasil"
        description="Dosen penguji mengisi hasil seminar hasil dan catatan/revisi untuk mahasiswa."
      />
      ...
      <section className="list-card">
        <div className="table-toolbar master-table-toolbar">
          <div>
            <h2>Daftar Hasil Seminar Hasil</h2>
            <p className="muted">
              Hasil LOLOS akan melanjutkan skripsi ke tahap menunggu Sidang Kompre.
            </p>
          </div>
        ...
        {sidangQuery.isLoading ? (
          <EmptyState
            title="Memuat hasil seminar hasil..."
                  ...
                  return (
                    <div className="table-actions">
                      {canInputHasil ? (
                        <button
                          type="button"
                          className={item.status === "SELESAI" ? "secondary-button" : "primary-button"}
                          disabled={!hasJadwal}
                          onClick={() => openHasilDrawer(item)}
                        >
                          {item.status === "SELESAI" ? "Ubah Hasil" : "Input Hasil"}
                        </button>
                      ) : (
                        <span className="muted">-</span>
                      )}
      ...
      {selectedSidang ? (
        <div className="crud-drawer-backdrop" role="presentation">
          <aside className="crud-drawer" aria-label="Hasil seminar hasil">
            <div className="crud-drawer-head">
              <div>
                <p className="eyebrow">Seminar Hasil</p>
                <h2>Input Hasil Seminar Hasil</h2>
              </div>
            ...
            <form className="form-stack" onSubmit={handleSubmit}>
              ...
              <label>
                ...
                <textarea
                  ...
                  placeholder="Catatan hasil seminar, revisi, atau arahan menuju Sidang Kompre."
                />
              </label>
