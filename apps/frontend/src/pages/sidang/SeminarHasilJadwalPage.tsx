...
export default function SeminarHasilJadwalPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return sidangRows.filter((item) =>
      ...
      } ${item.status ?? ""} ${item.hasil ?? ""} ${getPengujiLabels(item)} ${getTempat(item)}`
        .toLowerCase()
        .includes(keyword)
    );
  }, [sidangRows, search]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      closeDrawer();
      setPageError("");
      setSuccessMessage("Jadwal seminar hasil berhasil dibuat.");
    },
    onError: (error) => {
      setSuccessMessage("");
      setPageError(
        getApiErrorMessage(
          error,
          "Gagal membuat jadwal seminar hasil. Pastikan penguji sudah lengkap dan ruang tidak bentrok."
        )
      );
    }
  });
    ...
    return (
      <section className="page-stack">
        <div className="alert-error">
          Anda tidak memiliki akses ke halaman jadwal seminar hasil.
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="page-stack">
      <PageHeader
        ...
        description="Buat jadwal seminar hasil berdasarkan sidangId setelah penguji ditetapkan."
      />
      ...
      <section className="list-card">
        ...
        {sidangQuery.isLoading ? (
          <EmptyState
            title="Memuat jadwal seminar hasil..."
      ...
      {selectedSidang ? (
        <div className="crud-drawer-backdrop" role="presentation">
          <aside className="crud-drawer jadwal-drawer" aria-label="Jadwal seminar hasil">
            <div className="crud-drawer-head">
              <div>
                <p className="eyebrow">Seminar Hasil</p>
                <h2>Buat Jadwal Seminar Hasil</h2>
              </div>
